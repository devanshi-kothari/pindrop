// backend/db/reset_db.js
// Run this script to reset the Supabase database
// Usage: docker compose exec backend node backend/db/reset_db.js
...
async function resetDatabase() {
  ...
  try {
    ...
    // Get paths to SQL files (relative to project root)
    ...
    if (!fs.existsSync(resetSQLPath)) {
      throw new Error(`Reset SQL file not found: ${resetSQLPath}`);
    }
    ...
    await executeSQLFile(pool, resetSQLPath);

    console.log('Recreating tables from schema.sql...');
    await executeSQLFile(pool, schemaSQLPath);

    console.log('COMPLETE: Database reset complete!');
  } catch (error) {
    console.error('FAILED: Database reset failed:', error.message);
    process.exit(1);
  } finally {
    await pool.end();
  }
