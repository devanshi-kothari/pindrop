...
const ChatWindow = ({
  ...
  const [hasFetchedDepartureCodes, setHasFetchedDepartureCodes] = useState(false);
  const [hasFetchedArrivalCodes, setHasFetchedArrivalCodes] = useState(false);
  ...
  const [hasFetchedReturnArrivalCodes, setHasFetchedReturnArrivalCodes] = useState(false);
  ...
  const updateTripStatus = useCallback(
    async (newStatus: "planned" | "archived") => {
      if (!tripId) return;
      ...
      try {
        ...
        console.error("Error updating trip status from summary:", error);
  ...
  const getAirportCode = async (
    ...
    target: "outbound" | "return-departure" | "return-arrival" = "outbound"
  ): Promise<string | null> => {
    try {
      const token = getAuthToken();
      ...
      if (response.ok && result.success) {
        ...
        if (result.airport_codes && Array.isArray(result.airport_codes)) {
          ...
          } else if (target === "return-arrival") {
            setReturnArrivalAirportCodes(result.airport_codes);
            setSelectedReturnArrivalAirportCodes(result.airport_codes);
          } else if (isDeparture) {
        ...
        if (result.airports && Array.isArray(result.airports)) {
          if (target === "return-departure") {
            ...
            setReturnArrivalAirports(result.airports);
          } else if (isDeparture) {
          ...
          if (target === "return-departure") {
            ...
            setReturnArrivalAirports(fallbackAirports);
          } else if (isDeparture) {
            ...
            <div className="flex justify-center">
              <div className="w-full max-w-3xl bg-white border border-blue-100 text-slate-900 rounded-lg px-4 py-3 shadow-sm space-y-3">
                ...
                {!hasConfirmedFlights && (
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-2">
                      <Input
                        ...
                        disabled={isFetchingDepartureCode}
                      />
                      <Button
                        size="sm"
                        className="h-8 px-3 bg-blue-500 hover:bg-blue-600 text-xs text-white disabled:opacity-60"
                        disabled={!departureLocation.trim() || isFetchingDepartureCode}
                        onClick={async () => {
                          setDepartureId(null);
                          ...
                          if (code) {
                            ...
                            const errorMessage: Message = {
                              role: "assistant",
                              content:
                                "I couldn't find an airport code for that location. Please try a more specific location (e.g., 'New York, NY' or 'Austin, TX').",
                              timestamp: formatTime(),
                            };
                            setMessages((prev) => [...prev, errorMessage]);
                          }
                      ...
                      >
                        {isFetchingDepartureCode ? "Finding..." : "Find airport code"}
                      </Button>
                    </div>
                    {hasFetchedDepartureCodes && departureId && (
                          ...
                          <div className="space-y-1 pt-1 border-t border-blue-200">
                            ...
                            <div className="mt-1 grid grid-cols-2 gap-1 pl-1">
                              {(departureAirports.length > 0
                                ? departureAirports
                                : departureAirportCodes.map((code: string) => ({
                                    code,
                                    name: "",
                                    distance_miles: null,
                                  }))).map((airport) => {
                  ...
                  <div className="space-y-2">
                    <Label className="text-slate-800 text-[11px]">Outbound arrival location</Label>
                  {(() => {
                          ...
                          <>
                            ...
                            <div className="flex gap-2">
                              ...
                              <Button
                                size="sm"
                                className="h-8 px-3 bg-blue-500 hover:bg-blue-600 text-xs text-white disabled:opacity-60"
                        disabled={isFetchingArrivalCode}
                                onClick={async () => {
                            setArrivalId(null);
                            setArrivalAirportCodes([]);
                            setSelectedArrivalAirportCodes([]);
                              ...
                              >
                          {isFetchingArrivalCode ? "Finding..." : "Find airport code"}
                              </Button>
                            </div>
                            {hasFetchedArrivalCodes && arrivalId && (
                              <div className="space-y-1">
                              <p className="text-[11px] text-emerald-400">
                ...
                {orderedCities.length > 1 && (
                  <div className="space-y-3 rounded border border-blue-100 bg-blue-50/40 p-2">
                    <Label className="text-slate-800 text-[11px]">Return flight</Label>
                    <div className="space-y-2">
                      <Label className="text-slate-800 text-[11px]">Return departure location (final city)</Label>
                      <div className="flex items-center justify-between rounded border border-blue-100 bg-blue-50/60 px-2 py-1">
                        <span className="text-[11px] text-slate-600">Include nearby airports</span>
                        <Switch
                        ...
                        disabled={isFetchingReturnDepartureCode}
                          onClick={async () => {
                            ...
                            if (code) {
                              ...
                              setReturnDepartureAirportCodes((prev) => (prev.length > 0 ? prev : [code]));
                              setSelectedReturnDepartureAirportCodes((prev) => (prev.length > 0 ? prev : [code]));
                            } else {
                              const errorMessage: Message = {
                                role: "assistant",
                                content:
                            ...
                            <div className="space-y-1 pt-1 border-t border-blue-200">
                              ...
                              <div className="mt-1 grid grid-cols-2 gap-1 pl-1">
                                {(returnDepartureAirports.length > 0
                                  ...
                                  const code = airport.code;
                                  const isSelected = selectedReturnDepartureAirportCodes.includes(code);
                                  return (
                                    <button
                                      key={code}
                                      type="button"
                    ...
                    <div className="space-y-2">
                      <Label className="text-slate-800 text-[11px]">Return arrival location (home)</Label>
                      <div className="flex gap-2">
                        <Input
                          type="text"
                        ...
                        <Button
                          ...
                          setSelectedReturnArrivalAirportCodes([]);
                            ...
                            if (code) {
                              ...
                              const errorMessage: Message = {
                                role: "assistant",
                                content: "I couldn't find an airport code for that return arrival location.",
                                timestamp: formatTime(),
                              };
                              setMessages((prev) => [...prev, errorMessage]);
                            }
                        ...
                        >
                        {isFetchingReturnArrivalCode ? "Finding..." : "Find airport code"}
                        </Button>
                      </div>
                      {hasFetchedReturnArrivalCodes && returnArrivalId && (
                        <div className="space-y-1">
                          <p className="text-[11px] text-emerald-400">
                            ...
                            <div className="space-y-1 pt-1 border-t border-blue-200">
                              ...
                              <div className="mt-1 grid grid-cols-2 gap-1 pl-1">
                                {(returnArrivalAirports.length > 0
                                  ...
                                  const code = airport.code;
                                  const isSelected = selectedReturnArrivalAirportCodes.includes(code);
                                  return (
                                    <button
                                      key={code}
                                      type="button"
                    ...
                    {tripId && (
                      <div className="mt-6 space-y-3">
                        <div className="rounded-md border border-blue-100 bg-blue-50/60 px-3 py-3">
                          <p className="text-xs font-semibold text-slate-700">
                            Trip complete. Do you want to archive this trip?
                          </p>
                          <p className="text-[11px] text-slate-500 mt-1">
                            Ongoing/Upcoming keeps it active. Archive moves it to Past Trips.
                          </p>
                          {tripStatusActionError && (
                            <p className="mt-2 text-[11px] text-rose-500">{tripStatusActionError}</p>
                          )}
                          <div className="mt-3 flex flex-wrap items-center gap-2">
                            <Button
                              size="sm"
                              className="h-8 px-3 bg-blue-500 text-white hover:bg-blue-600 disabled:opacity-60"
                              onClick={() => void updateTripStatus("planned")}
                              disabled={isUpdatingTripStatus}
                            >
                              {isUpdatingTripStatus ? "Saving..." : "Keep in Ongoing/Upcoming"}
                            </Button>
                            <Button
                              size="sm"
                              variant="outline"
                              className="h-8 px-3 border-slate-300 bg-white text-xs text-slate-900 hover:bg-slate-50 disabled:opacity-60"
                              onClick={() => void updateTripStatus("archived")}
                              disabled={isUpdatingTripStatus}
                            >
                              Archive to Past Trips
                            </Button>
                          </div>
                        </div>
                        <div className="flex justify-end">
